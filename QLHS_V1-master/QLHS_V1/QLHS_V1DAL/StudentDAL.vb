...
Public Class StudentDAL
    ...
    Public Function insert(hs As StudentDTO) As Result
        ...
        Dim query As String = String.Empty
        query &= "INSERT INTO [tbl_Student] ([id_Student], [ID_Gender], [student_name], [address], [brithday],[name_CLASS])"
        'sai fix
        query &= "VALUES (@id_Student,@ID_Gender,@student_name,@address,@brithday,@name_CLASS)"
        ...
        Using conn As New SqlConnection(connectionString)
            Using comm As New SqlCommand()
                With comm
                    ...
                    'them ten lop
                    .Parameters.AddWithValue("@name_CLASS", hs.nameclass)
                End With
                Try
                    conn.Open()
            ...
            Using comm As New SqlCommand()
                ...
                Try
                    ...
                    If reader.HasRows = True Then
                        listStudent.Clear()
                        While reader.Read()
                            listStudent.Add(New StudentDTO(reader("ID_student"), reader("Gender"), reader("Student_Name"), reader("Address"), reader("DateTime"), reader("nameclass")))
                        End While
                    End If
            ...
            Using comm As New SqlCommand()
                ...
                Try
                    ...
                    If reader.HasRows = True Then
                        listHocSinh.Clear()
                        While reader.Read()
                            listHocSinh.Add(New StudentDTO(reader("ID_student"), reader("Gender"), reader("Student_Name"), reader("Address"), reader("DateTime"), reader("nameclass")))
                        End While
                    End If
                ...
                Catch ex As Exception
                    conn.Close()
                    System.Console.WriteLine(ex.StackTrace)
                    Return New Result(False, "Taking all students by grade failed", ex.StackTrace)
                End Try
            End Using
        End Using
        Return New Result(True) ' thanh cong
